...
from maze_generator import (generate_maze, load_maze_from_json,
                            save_maze_to_json)
from maze_lib import (BFS, DFS, AStar, Backtracking, BellmanFord, CellType,
                      Dijkstra, Maze)
...
def update_canvas(root, canvas, maze: Maze):
    ...
    # Update more frequently (60 FPS)
    root.after(16, update_canvas, root, canvas, maze)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Maze visualization and pathfinding')
    parser.add_argument('--load', '-l', type=str,
                        help='Load maze from specified file path')
    ...
    if args.load:
        ...
        if loaded_maze:
            ...
            print(
                f"Failed to load maze from {args.load}, generating new maze instead")
    ...
    def save_maze():
        ...
        if file_path:
            # Using the function from maze_generator
            save_maze_to_json(maze, file_path)
    ...
    def load_maze():
        ...
        if file_path:
            nonlocal maze
            # Using the function from maze_generator
            loaded_maze = load_maze_from_json(file_path)
            if loaded_maze:
                maze = loaded_maze
    ...
    def update_canvas_wrapper():
        ...
        draw_maze(canvas, maze)
        # Call wrapper instead of original function
        root.after(16, update_canvas_wrapper)
